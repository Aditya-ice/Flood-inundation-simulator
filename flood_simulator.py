...
class FloodSimulatorAdvanced:
    ...
    def get_units_label(self) -> str:
        if self.crs:
            try:
                if self.crs.linear_units_factor and self.crs.linear_units_factor != 0: return self.crs.linear_units
            except AttributeError:
                pass
            if self.crs.is_projected: return "units (projected)"
            if self.crs.is_geographic: return "degrees (approx.)"
        return "m"
    ...
    def multi_level_analysis(self, water_levels: List[float],
        ...
        for level in water_levels:
            ...
            volume = self.calculate_flood_volume(depth_map);
...
def main_flood_analysis_for_city():
    ...
    downloader = DEMDataDownloader(data_dir=str(master_dem_download_dir))
    geocoded_bounds = geocode_city(city_name)

    dem_filepath = None
    if geocoded_bounds:
        dem_filename = f"{safe_city_name}_srtm_dem.tif"
        dem_filepath = downloader.download_srtm_data(geocoded_bounds, output_filename=dem_filename)
    else:
